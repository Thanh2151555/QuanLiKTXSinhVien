...
public class StudentNotificationController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet StudentNotificationController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet StudentNotificationController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Notification> notificationList = dao.getNotificationByIdUser(user.getId(), offset);
        boolean hasMore = false;

        if (notificationList.size() > 2) {
            hasMore = true;
            notificationList.remove(2); // chỉ giữ lại 2 cái đầu
        }
